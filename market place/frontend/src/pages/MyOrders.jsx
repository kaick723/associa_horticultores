        ...
        <div className="list-group">
          {orders.map((order) => (
            <div key={order._id} className="list-group-item">
              ...
              <p>Status: {statusPt[order.status] || order.status}</p>
              <p>
                Endereço: {order.deliveryAddress.address},{" "}
                {order.deliveryAddress.number} - CEP:{" "}
              ...
              <ul>
                {order.items.map((item, i) => (
                  <li key={i}>
                    {item.name} - {item.qty} x R$ {item.price.toFixed(2)}
                  </li>
                ))}
              </ul>
            </div>
